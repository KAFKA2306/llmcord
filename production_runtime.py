...
def validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if deployment.get("network_scope") != "loopback" or deployment.get("api_host") != "127.0.0.1":
        raise ProductionContractError("production inference API must be loopback-only on 127.0.0.1")
    ...
    backend_service = _string(deployment.get("backend_service"), "production.deployment.backend_service")
    bot_service = _string(deployment.get("bot_service"), "production.deployment.bot_service")
    ...
    if bot_service != "llmcord.service":
        raise ProductionContractError("bot service authority must be llmcord.service")

    if backend.get("name") != "llama.cpp":
        raise ProductionContractError("production backend must be llama.cpp")
    version = _string(backend.get("version"), "production.backend.version")
    if version in {"latest", "main", "master"} or re.fullmatch(r"v\d+\.\d+\.\d+", version) is None:
        raise ProductionContractError("production backend version must be an explicit release version")
    commit = _string(backend.get("commit"), "production.backend.commit")
    if re.fullmatch(r"[0-9a-f]{40}", commit) is None:
        raise ProductionContractError("production backend commit must be a full 40-character SHA")
    binary = _string(backend.get("binary"), "production.backend.binary")
    if Path(os.path.expanduser(binary)).name != "llama-server":
        raise ProductionContractError("production backend binary must be llama-server")
    context_window = _positive_int(backend.get("context_window_tokens"), "production.backend.context_window_tokens")
    ...
    model_key = _string(production.get("model_key"), "production.model_key")
    model_alias = _string(model.get("alias"), "production.model.alias")
    if model_key != f"llamacpp/{model_alias}":
        raise ProductionContractError("production.model_key must be the llamacpp provider plus model alias")
    _string(model.get("upstream"), "production.model.upstream")
    artifact_repo = _string(model.get("artifact_repo"), "production.model.artifact_repo")
    revision = _string(model.get("revision"), "production.model.revision")
    ...
    filename = _string(model.get("filename"), "production.model.filename")
    sha256 = _string(model.get("sha256"), "production.model.sha256")
    if re.fullmatch(r"[0-9a-f]{64}", sha256) is None:
        raise ProductionContractError("production model sha256 must be 64 lowercase hex characters")
    expected_url = f"https://huggingface.co/{artifact_repo}/resolve/{revision}/{filename}"
    if _string(model.get("artifact_url"), "production.model.artifact_url") != expected_url:
        raise ProductionContractError("production model artifact_url must use the pinned repo/revision/filename")
    model_path = _string(model.get("path"), "production.model.path")
    if Path(os.path.expanduser(model_path)).name != filename:
        raise ProductionContractError("production model path filename must match the pinned artifact filename")
    ...
    device_index = _non_negative_int(gpu.get("device_index"), "production.gpu.device_index")
    min_vram = _positive_int(gpu.get("min_vram_used_mib"), "production.gpu.min_vram_used_mib")
    ...
    runtime = _mapping(config, "runtime_control")
    if runtime.get("max_concurrency") != parallel:
        raise ProductionContractError("runtime max_concurrency must match production backend parallel")
    ...
    context = _mapping(_mapping(models, model_key), "context_management")
    ...
    if health.get("enabled") is not True or health.get("model") != model_key:
        raise ProductionContractError("production watchdog must be enabled for the pinned model")
    ...
    if health_gpu.get("device_index") != device_index or health_gpu.get("min_vram_used_mib") != min_vram:
        raise ProductionContractError("watchdog GPU contract must match production GPU authority")
...
def backend_argv(config: dict[str, Any]) -> list[str]:
    ...
    return [
        str(expand_path(backend["binary"])),
        "--model",
        str(expand_path(model["path"])),
        "--alias",
        model["alias"],
        "--host",
        deployment["api_host"],
...
def verify_model_artifact(config: dict[str, Any]) -> None:
    model = validate_config(config)["model"]
    ...
    if actual != model["sha256"]:
        raise ProductionContractError(f"production model SHA256 mismatch: expected {model['sha256']}, got {actual}")


def verify_backend_binary(config: dict[str, Any]) -> None:
    backend = validate_config(config)["backend"]
    ...
    expected_version = re.escape(backend["version"].removeprefix("v"))
    if re.search(rf"\b{expected_version}\b", output) is None:
