...
連続 failure が2回に達したら受付を閉じ、systemd backend service を restart します。restart 後は NVIDIA GPU の selected device / VRAM / expected `llama-server` compute process と synthetic generation probe が成功して初めて受付を再開します。
